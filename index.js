...
const default_volume = process.env.NONO_VOLUME
  ? parseFloat(process.env.NONO_VOLUME)
  : 0.6;
const volume_scale = isNaN(default_volume)
  ? 0.6
  : Math.max(0, Math.min(1, default_volume));
const default_output_limit = process.env.NONO_SUMMARIZE_OUTPUT_LIMIT
  ? parseInt(process.env.NONO_SUMMARIZE_OUTPUT_LIMIT, 10)
  : 10000;
const output_limit = isNaN(default_output_limit) ? 10000 : default_output_limit;
const default_thought_limit = process.env.NONO_THOUGHT_LIMIT
  ? parseInt(process.env.NONO_THOUGHT_LIMIT, 10)
  : 120;
const thought_limit = isNaN(default_thought_limit)
  ? 120
...
function cleanModelText(text) {
  ...
  cleaned = cleaned.replace(
    /<\/?(tool_call|function|parameter)(=[a-zA-Z0-9_-]+)?>/gi,
    "",
  );
  ...
  while ((match = tool_call_regex.exec(text)) !== null) {
    ...
    if (func_match) {
      ...
      while ((param_match = param_regex.exec(inner)) !== null) {
        ...
        else if (!isNaN(param_val) && param_val !== "")
          args[param_name] = Number(param_val);
        else if (
          (param_val.startsWith('"') && param_val.endsWith('"')) ||
          (param_val.startsWith("'") && param_val.endsWith("'"))
        )
          args[param_name] = param_val.slice(1, -1);
...
const gemini_web_search_declaration = {
  ...
  description:
    "Queries Google Search using Gemini's web search API to retrieve up-to-date information, news, and details from the web.",
  parameters: {
    type: "OBJECT",
    properties: {
      query: {
...
function drawBottomLine() {
  ...
  let token_limit = use_vllm
    ? vllm_max_context
    : parseInt(process.env.NONO_SUMMARIZE_TOKEN_LIMIT, 10) || 40000;

  if (use_vllm && latest_vllm_stats?.vllm) {
    current_tokens =
      latest_vllm_stats.vllm.current_context_tokens_total ||
      latest_context_size ||
      0;
    token_limit = latest_vllm_stats.vllm.max_model_len || token_limit;
    if (
      is_talking_active &&
      latest_vllm_stats.vllm.generation_tokens_total !== undefined
    ) {
      if (vllm_baseline_generation === null) {
        vllm_baseline_generation =
          latest_vllm_stats.vllm.generation_tokens_total;
      }
      talking_token_count = Math.max(
        0,
        latest_vllm_stats.vllm.generation_tokens_total -
          vllm_baseline_generation,
      );
      if (vllm_start_time) {
        ...
        latest_tok_speed =
          elapsed_s > 0 ? Math.round(talking_token_count / elapsed_s) : 0;
      }
    }
  }

  const pct =
    token_limit > 0 ? Math.round((current_tokens / token_limit) * 100) : 0;
  const formatted_current = formatK(current_tokens);
  const formatted_limit = formatK(token_limit);
  const speed_str =
    latest_tok_speed > 0 ? `${latest_tok_speed} tok/s` : "-- tok/s";

  const parts = [
    `${formatted_current} / ${formatted_limit} (${pct}%)`,
    speed_str,
  ];
  ...
  if (use_vllm && latest_vllm_stats) {
    if (latest_vllm_stats.cpu) {
      parts.push(
        `CPU ${Math.round(latest_vllm_stats.cpu.usage_percentage)}% ${Math.round(latest_vllm_stats.cpu.temperature_celsius)}°C`,
      );
    }
    if (latest_vllm_stats.gpus && Array.isArray(latest_vllm_stats.gpus)) {
      latest_vllm_stats.gpus.forEach((gpu) => {
        parts.push(
          `GPU${gpu.index} ${Math.round(gpu.usage_percentage)}% ${Math.round(gpu.temperature_celsius)}°C`,
        );
      });
    }
  }
...
function extractJsonBlock(text) {
  ...
  writeDetails(
    `[JSON Parse Failure] Failed to parse JSON block from: \n${text}\n`,
  );
  return null;
}
    ...
    if (message && message.role === "user" && Array.isArray(message.parts)) {
      for (const part of message.parts) {
        if (part && part.functionResponse && part.functionResponse.response) {
          ...
          if (
            part.functionResponse.name === "view_file_contents" &&
          ...
          if (
            part.functionResponse.name === "search_grep" &&
            typeof response.matches === "string"
          ) {
            if (response.matches.length > 1000) {
              response.matches =
                response.matches.slice(0, 1000) +
          ...
          if (part.functionResponse.name === "execute_system_command") {
            if (
              ...
              response.stdout.length > 1000
            ) {
              response.stdout =
                response.stdout.slice(0, 1000) +
                "\n[... stdout truncated in history pruning ...]";
              response.stdout_truncated = true;
            }
            if (
              typeof response.stderr === "string" &&
              response.stderr.length > 1000
            ) {
              response.stderr =
          ...
          if (
            part.functionResponse.name === "view_file_git_diff" &&
            typeof response.diff === "string"
          ) {
            if (response.diff.length > 1000) {
              response.diff =
                response.diff.slice(0, 1000) +
  ...
  return history.filter(Boolean).map((msg) => {
    ...
    if (!msg.role) {
      const hasFunctionResponse =
        ...
        msg.parts.some((p) => p && p.functionResponse);
...
async function handleBackgroundSummarization(session_path) {
  ...
  for (let i = 0; i < history.length; i++) {
    ...
    if (msg && msg.role === "user") {
      ...
      if (
        first_part &&
        typeof first_part.text === "string" &&
        first_part.text.startsWith("[System Memory:\n")
      )
        continue;
  ...
  try {
    ...
    if (summary && fs.existsSync(session_path)) {
      let current_history = [];
      try {
        current_history = sanitizeHistory(
          JSON.parse(fs.readFileSync(session_path, "utf8")),
        );
      } catch (e) {
      ...
      if (Array.isArray(current_history)) {
        ...
        fs.writeFileSync(
          session_path,
          JSON.stringify(new_history, null, 2),
          "utf8",
        );
      }
    }
...
async function ensureContextLimit(history, session_path) {
  ...
  try {
    ...
    if (total_tokens > threshold && user_turns >= 3) {
      console.log(
        `\x1b[90m• Session history is growing large (${total_tokens} tokens). Compressing...\x1b[0m`,
      );
      await handleBackgroundSummarization(session_path);
      const new_history = sanitizeHistory(
        JSON.parse(fs.readFileSync(session_path, "utf8")),
      );
      history.length = 0;
      ...
      console.log(
        `\x1b[90m• Reduced to ${formatK(new_total_tokens)} tokens\x1b[0m`,
      );
    }
  } catch (e) {
    // Fail silently
...
function loadCustomTheme() {
  ...
  if (theme_json_str) {
    try {
      ...
      writeDetails(
        `[Theme Load Error] Failed to parse custom theme JSON: ${err.message}`,
      );
    }
  }
  return undefined;
  ...
  switch (name) {
    case "list_directory_structure": {
      const dir = args.directory_path
        ? path.basename(args.directory_path) || args.directory_path
        : ".";
    ...
    case "run_node_script": {
      ...
      const snippet =
        first_line.length > 50 ? first_line.slice(0, 47) + "..." : first_line;
...
async function formatMarkdownForTerminal(md, options = {}) {
  ...
  for (let line of lines) {
    ...
    if (line.trim().startsWith("```")) {
      if (!in_code_block) {
        ...
        const is_highlighted =
          code_block_lang && cliHighlight.supportsLanguage(code_block_lang);
        let highlighted_text = code_text;
        if (is_highlighted) {
          try {
            const formatted_code = await formatCodeWithPrettier(
              code_text,
              code_block_lang,
            );
        ...
        for (const h_line of highlighted_lines) {
          if (is_highlighted) {
            ...
            formatted_lines.push(
              `  \x1b[90m│\x1b[0m  \x1b[37m${h_line}\x1b[0m`,
            );
          }
        }
      }
    ...
    if (quote_match) {
      ...
      formatted_lines.push(
        `${indent}  \x1b[90m│\x1b[0m  \x1b[90m${processed_content}\x1b[0m`,
      );
      continue;
    }
...
function findSessionModelMessages() {
  ...
  const sessionFiles = files
    .filter(
      (file) =>
        (file.startsWith("session-") || file.startsWith("session-pr-")) &&
        file.endsWith(".json"),
    )
    .map((file) => {
  ...
  for (const sessionFile of sessionFiles) {
    try {
      const history = sanitizeHistory(
        JSON.parse(fs.readFileSync(sessionFile.path, "utf8")),
      );
...
async function finishProgress(final_text, grounding_sources) {
  ...
  const formatted = await formatMarkdownForTerminal(
    cleanModelText(final_text).trim(),
  );
  console.log();
  ...
  const avg_tok_speed =
    total_api_duration_ms > 0
      ? Math.round(
          total_candidates_token_count / (total_api_duration_ms / 1000),
        )
      : 0;
...
function playChime(type) {
  ...
  tones.forEach(
    (t) => (t.gain = (t.gain !== undefined ? t.gain : 0.15) * volume_scale),
  );
  ...
  try {
    ...
    const player = fs.existsSync("/usr/bin/pw-play")
      ? "pw-play"
      : fs.existsSync("/usr/bin/paplay")
        ? "paplay"
        : fs.existsSync("/usr/bin/aplay")
          ? "aplay"
          : null;
  ...
  return new Promise((resolve) => {
    const rl = readline.createInterface({
      ...
      completer: function completer(line) {
        ...
        if (
          lastAtIdx !== -1 &&
          (lastAtIdx === 0 || /\s/.test(line[lastAtIdx - 1]))
        ) {
    ...
    const keypressHandler = (str, key) => {
      try {
        if (
          ...
          (key && (key.name === "escape" || key.name === "q"))
        ) {
...
function readTerminalBuffer() {
  ...
  if (buffer === null) {
    return {
      status: "error",
      message:
        "Could not read terminal buffer (e.g. remote control disabled, or not in kitty)",
    };
  }
  return {
    status: "success",
...
function isHighImpactCommand(command) {
  ...
  if (
    normalized.includes("pacman") ||
    normalized.includes("yay") ||
    normalized.includes("paru")
  )
    return true;
  ...
  if (
    ...
    const is_write =
      />|>>|tee|rm\s|mv\s|cp\s|chmod|chown|edit|mkdir|touch/g.test(command);
...
function getPrettierFlagsFromVSCode() {
  ...
  try {
    ...
    const config_mapping = {
      ...
      "prettier.singleQuote": (val) =>
        val ? "--single-quote" : "--no-single-quote",
      ...
      "prettier.bracketSpacing": (val) =>
        val ? "--bracket-spacing" : "--no-bracket-spacing",
      "prettier.jsxSingleQuote": (val) =>
        val ? "--jsx-single-quote" : "--no-jsx-single-quote",
...
function getLineDiff(oldStr, newStr) {
  ...
  try {
    ...
    const oldTempPath = path.join(
      tempDir,
      `nono_diff_old_${Date.now()}_${Math.random().toString(36).substring(2, 15)}.txt`,
    );
    const newTempPath = path.join(
      tempDir,
      `nono_diff_new_${Date.now()}_${Math.random().toString(36).substring(2, 15)}.txt`,
    );
    ...
    try {
      ...
      try {
        stdout = execSync(
          `git diff --no-index --numstat ${JSON.stringify(oldTempPath)} ${JSON.stringify(newTempPath)}`,
          {
            encoding: "utf8",
...
function writeFile({ file_path, content }) {
  ...
  updateProgress(
    `• Writing ${path.basename(file_path)} \x1b[31m-${deleted}\x1b[90m \x1b[32m+${added}\x1b[90m`,
  );
...
function patchFile({ file_path, search_block, replace_block }) {
  ...
  if (index !== last_index) {
    throw new Error(
      `Search block is not unique. It appears multiple times in file: ${file_path}`,
    );
  }

  const patched_content =
    normalized_content.slice(0, index) +
    normalized_replace +
    normalized_content.slice(index + normalized_search.length);
  ...
  const { deleted, added } = getLineDiff(old_content, final_content);
  updateProgress(
    `• Patching ${path.basename(file_path)} \x1b[31m-${deleted}\x1b[90m \x1b[32m+${added}\x1b[90m`,
  );
...
function searchGrep({ pattern, directory_path }) {
  return new Promise((resolve) => {
    const search_dir = directory_path
      ? path.resolve(directory_path)
      : process.cwd();
...
async function executeSystemCommand({ command, timeout_ms = 30000 }) {
  if (isHighImpactCommand(command) && !allow_all_high_impact) {
    ...
    const answer = await askUserInRoll(
      `Do you want to run this command? [Y/n/a]: `,
    );
  ...
  if (command.includes("sudo")) {
    try {
      ...
      updateProgress(
        `• sudo credential caching required. Please authenticate when prompted:`,
        "red",
      );
...
function proposeTerminalInput({ command_to_inject }) {
  if (!is_kitty) {
    return Promise.resolve({
      status: "error",
      error:
        "Could not propose terminal input (Kitty terminal is not detected)",
    });
  }
  return new Promise((resolve) => {
...
function findCorrespondingCall(history, userMsgIndex, responsePart, partIndex) {
  ...
  if (responsePart.functionResponse.id) {
    const found = modelMsg.parts.find(
      (p) =>
        p.functionCall &&
        p.functionCall.id === responsePart.functionResponse.id,
    );
  ...
  if (
    ...
    modelPart.functionCall.name === responsePart.functionResponse.name
  ) {
    return modelPart.functionCall;
  }

  // 3. Fallback: match by name
  const foundByName = modelMsg.parts.find(
    (p) =>
      p.functionCall &&
      p.functionCall.name === responsePart.functionResponse.name,
  );
        ...
        if (part && part.functionResponse) {
          ...
          if (call && matchesTarget(call, target)) {
            part.functionResponse.response = {
              ...
              erased: true,
              message:
                "This output has been erased to optimize context window space.",
            };
            count++;
          }
        }
...
const tools_declarations = [
    ...
    parameters: {
      ...
      properties: {
        target: {
          type: "STRING",
          description:
            "The exact file path or command string whose output should be cleared from memory.",
        },
      },
      required: ["target"],
    },
  },
  {
    ...
    description:
      "Replaces the outputs of the last N tool calls with an 'erased' placeholder. Use this to quickly clean up memory after realizing your most recent steps or explorations were a dead end.",
    parameters: {
      type: "OBJECT",
      properties: {
        steps_count: {
          type: "INTEGER",
          description:
            "The number of recent tool outputs to erase from memory (counting backwards from the most recent call).",
        },
      },
  ...
  },
  {
    ...
    description:
      "Lists the files and folders in a directory recursively up to a certain depth to understand the project workspace layout.",
    parameters: {
      type: "OBJECT",
      properties: {
        directory_path: {
  ...
  },
  {
    ...
    description:
      "Reads the content of a file. Supports line-range targeting. Note: Outputs exceeding 30,000 characters will be truncated.",
    parameters: {
      type: "OBJECT",
      properties: {
  ...
  },
  {
    name: "write_file",
    description:
      "Creates a new file or overwrites an existing file with complete fresh content.",
    parameters: {
      type: "OBJECT",
      properties: {
        file_path: {
  ...
  },
  {
    name: "patch_file",
    description:
      "Applies a specific diff, line replacement, or block modification to a file to minimize rewriting huge files.",
    parameters: {
      type: "OBJECT",
      properties: {
  ...
  },
  {
    name: "search_grep",
    description:
      "Performs a fast regex-based substring search across the workspace (equivalent to ripgrep) to find references or declarations. Note: Outputs exceeding 30,000 characters will be truncated.",
    parameters: {
      type: "OBJECT",
      properties: {
        pattern: {
    ...
    parameters: {
      ...
      properties: {
        command: {
          type: "STRING",
          description:
            "The exact terminal command to run (e.g. 'nmcli dev wifi list', 'cargo build').",
        },
        timeout_ms: {
          type: "INTEGER",
          description:
    ...
    ? [
        {
          ...
          description:
            "Injects text straight into the user's active Zsh prompt using Kitty's remote control feature, leaving the user to hit Enter.",
          parameters: {
            type: "OBJECT",
            properties: {
              command_to_inject: {
                type: "STRING",
                description:
                  "The command string to stage on the user shell line.",
              },
            },
        ...
        },
        {
          ...
          description:
            "Reads the active terminal buffer history (last 100 lines) from the Kitty terminal.",
          parameters: {
            type: "OBJECT",
            properties: {},
          },
...
const view_file_git_diff_declaration = {
  ...
  description:
    "Shows the line-by-line git diff of a specific file in the PR branch compared to the base branch, or the entire PR git diff if file_path is omitted.",
  parameters: {
    type: "OBJECT",
    properties: {
      base_branch: {
      ...
      file_path: {
        type: "STRING",
        description:
          "The relative path of the file to inspect. If omitted, returns the diff for all changed files.",
      },
    },
...
function logTokenUsage(model, usageMetadata, prompt) {
  ...
  if (
    loggedPrompt.startsWith(
      "Perform a pull request review for the Github Pull Request:",
    )
  ) {
    const match = loggedPrompt.match(
      /Perform a pull request review for the Github Pull Request:\s*([^\n\s.]+)/,
    );
    if (match) {
...
async function main() {
  ...
  if (use_vllm && openai) {
    try {
      ...
      if (models.data && models.data.length > 0) {
        ...
        const matchedModel =
          models.data.find((m) => m.id === vllm_model_name) || models.data[0];
    ...
    if (
      process.argv[2] !== "--summarize-background" &&
      ![
        "--details",
        "--usage",
        "--help",
      ...
      console.warn(
        "\x1b[33mWarning: The Gemini API will be used for the current task.\x1b[0m",
      );
    }
  }
  ...
  const autoContinueIdx = process.argv.findIndex(
    (arg, i) => i >= 2 && (arg === "-ac" || arg === "--auto-continue"),
  );
  ...
  if (
    fs.existsSync(prMetaPath) &&
    process.argv[2] !== "--clear" &&
    process.argv[2] !== "--pr-review" &&
    process.argv[2] !== "pr-review" &&
...
`);
  ...
  if (process.argv[2] === "--list-instructions") {
    ...
    if (files.length === 0) {
      console.log(
        "No nono.md files found in the current folder or any of its parent folders.",
      );
    } else {
  ...
  if (process.argv[2] === "--get-pricing") {
    console.log(
      "\x1b[35m✦ Fetching current pricing and country information...\x1b[0m\n",
    );
...
Return ONLY a JSON object. Do not include markdown code block formatting (like \`\`\`json). The JSON object MUST have the following structure:
{
  ...
  "cache_price_per_m": <number>
    ...
    try {
      ...
      let text =
        pricingResponse.candidates?.[0]?.content?.parts?.[0]?.text || "";
      ...
      try {
        ...
        console.error(
          "\x1b[31mError: Failed to parse pricing response from Gemini.\x1b[0m",
        );
      ...
      if (
        isNaN(newPriceInput) ||
        isNaN(newPriceOutput) ||
        isNaN(newPriceCache)
      ) {
        console.error(
          "\x1b[31mError: Pricing response did not return valid numeric values.\x1b[0m",
        );
      ...
      const currentPriceInput =
        parseFloat(
          process.env.NONO_PRICE_INPUT_PER_M ||
            process.env.NONO_PRICE_INPUT_EUR_PER_M,
        ) || 1.38;
      const currentPriceOutput =
        parseFloat(
          process.env.NONO_PRICE_OUTPUT_PER_M ||
            process.env.NONO_PRICE_OUTPUT_EUR_PER_M,
        ) || 8.28;
      const currentPriceCache =
        parseFloat(
          process.env.NONO_PRICE_CACHE_PER_M ||
            process.env.NONO_PRICE_CACHE_EUR_PER_M,
        ) || 0.138;
      ...
      const pad = (str, length) =>
        str + " ".repeat(Math.max(0, length - String(str).length));
      const padLeft = (str, length) =>
        " ".repeat(Math.max(0, length - String(str).length)) + str;

      console.log(
        `${pad("Input (non-cached)", 19)} │ ${padLeft(`${currentPriceInput.toFixed(2)}${currency}`, 13)} │ ${padLeft(`${newPriceInput.toFixed(2)}${currency}`, 15)}`,
      );
      console.log(
        `${pad("Cache Hit", 19)} │ ${padLeft(`${currentPriceCache.toFixed(2)}${currency}`, 13)} │ ${padLeft(`${newPriceCache.toFixed(2)}${currency}`, 15)}`,
      );
      console.log(
        `${pad("Output", 19)} │ ${padLeft(`${currentPriceOutput.toFixed(2)}${currency}`, 13)} │ ${padLeft(`${newPriceOutput.toFixed(2)}${currency}`, 15)}`,
      );
      console.log(`────────────────────┴───────────────┴─────────────────`);
      ...
      if (norm === "y" || norm === "yes") {
        ...
        console.log(
          "\x1b[32m✔ Pricing values updated successfully in .env file!\x1b[0m\n",
        );
      } else {
      ...
      console.error(
        `\x1b[31mError during pricing lookup: ${err.message || err}\x1b[0m`,
      );
      process.exit(1);
    }
    process.exit(0);
  ...
  if (process.argv[2] === "--clear") {
    ...
    const session_pr_path = path.join(
      cache_dir,
      `session-pr-${process.ppid}.json`,
    );
    ...
    try {
      ...
      console.log(
        "\x1b[32m✔ Nono session cleared. Ready for a new chat!\x1b[0m\n",
      );
    } catch (err) {
  ...
  if (process.argv[2] === "--resume") {
    ...
    let selectedIndex;
    try {
      selectedIndex = await chooseOption(
        options,
        "\x1b[35mSelect a session to resume:\x1b[0m",
      );
    } catch (e) {
      console.error(e);
      process.exit(1);
    ...
    const currentSessionPath = session.file.startsWith("session-pr-")
    ...
    console.log(
      `\x1b[32mSession context loaded! The next nono command will continue this session.\x1b[0m\n`,
    );
  ...
  if (process.argv[2] === "--commit") {
    ...
    try {
      ...
      console.log(
        "\x1b[31m✦ Not a git repository (or any of the parent directories)\x1b[0m",
      );
      process.exit(1);
    }
    ...
    if (!diff) {
      console.log(
        '\x1b[31m✦ No staged changes found. Use "git add" to stage files first.\x1b[0m',
      );
      process.exit(1);
    }

    console.log(
      "\x1b[35m✦ Generating git commit message suggestions...\x1b[0m\n",
    );
...
fix: resolve null pointer exception in checkout flow`;
      ...
      if (suggestions.length === 0) {
        console.log(
          "\x1b[31m✦ Could not generate suggestions. Please try again or write your own message.\x1b[0m",
        );
        process.exit(1);
      }
      ...
      const selectedIdx = await chooseOption(
        options,
        "\x1b[35mSelect a git commit message suggestion:\x1b[0m",
      );
      let commitMessage = "";
      ...
      if (selectedIdx === suggestions.length) {
        ...
        if (!commitMessage) {
          console.log(
            "\x1b[31m✦ Commit message cannot be empty. Cancelled.\x1b[0m",
          );
          process.exit(1);
        }
      } else {
      ...
      console.error(
        `\x1b[31mError during commit generation or execution: ${err.stdout || err.message}\x1b[0m`,
      );
      process.exit(1);
    }
  ...
  if (process.argv[2] === "--usage") {
    ...
    let listCount = null;
    const listIdx = process.argv.findIndex(
      (arg) => arg === "--list" || arg === "-l",
    );
    if (listIdx !== -1) {
      listCount = 10;
    ...
    if (listCount !== null) {
      ...
      for (const log of logs) {
        ...
        if (shouldGroup && currentGroup) {
          ...
          currentGroup.cachedContentTokenCount +=
      ...
      for (const log of lastLogs) {
        ...
        const inputVal =
          (log.promptTokenCount || 0) - (log.cachedContentTokenCount || 0);
        ...
        if (!displayPrompt) {
          if (log.cachedContentTokenCount > 0 || log.promptTokenCount > 5000) {
            ...
            displayPrompt = legacyPrName
              ? `PR review ${legacyPrName}`
              : "PR review";
          } else {
    ...
    const elapsedFraction = Math.max(
      0.0001,
      (now - startOfMonth) / (nextMonth - startOfMonth),
    );

    for (const log of logs) {
      const logDate = new Date(log.timestamp);
      const isInCurrentMonth =
        logDate.getFullYear() === year && logDate.getMonth() === month;
    ...
    const sessionTotalCost =
      sessionCostInput + sessionCostCache + sessionCostOutput;
    ...
    // ----------------------------------------------------
    console.log(
      `\x1b[1;35m✦ Session Consumption (PPID: ${process.ppid})\x1b[0m`,
    );
    ...
    printRow1(
      "Input (non-cached)",
      `${priceInput.toFixed(2)}${currency}`,
      sessionInput,
      sessionCostInput,
    );
    printRow1(
      "Cache Hit",
      `${priceCache.toFixed(2)}${currency}`,
      sessionCache,
      sessionCostCache,
    );
    printRow1(
      "Output",
      `${priceOutput.toFixed(2)}${currency}`,
      sessionOutput,
      sessionCostOutput,
    );
    ...
    console.log(
      `\x1b[1;35m✦ Monthly Consumption & Projections (${monthName} ${year})\x1b[0m`,
    );
  ...
  if (process.argv[2] === "--raw") {
    ...
    if (messages.length > 0) {
      ...
      console.error(
        "\x1b[31mError: No previous final message found in session history.\x1b[0m",
      );
      process.exit(1);
    }
    return;
  ...
  if (process.argv[2] === "--pr-review" || process.argv[2] === "pr-review") {
    ...
    if (!prUrl) {
      ...
      console.error(
        "Usage: nono --pr-review <github-pr-url> [--comment] [--auto]",
      );
      playChime("error");
      process.exit(1);
    }
    ...
    isCommentMode =
      process.argv.includes("--comment") || process.argv.includes("--auto");
    ...
    console.log(
      `\x1b[35m✦ Initiating Github PR Review for ${owner}/${repo}#${pullNumber}...\x1b[0m\n`,
    );
    ...
    githubFetch = async function (url, options = {}) {
      ...
      if (!response.ok) {
        ...
        const acceptedScopes =
          response.headers.get("x-accepted-oauth-scopes") || "none";
        throw new Error(
    ...
    try {
      ...
      const filesData = await githubFetch(
        `https://api.github.com/repos/${owner}/${repo}/pulls/${pullNumber}/files`,
      );

      const filteredFiles = filesData.filter(
        (file) => !isIgnoredFile(file.filename),
      );

      updateProgress(
        `• Cloning repository branch "${compareBranch}" for analysis...`,
      );
      tempDir = path.join(
        os.tmpdir(),
        `nono-pr-${owner}-${repo}-${pullNumber}-${Date.now()}`,
      );
      ...
      try {
        repoDiff = execSync(
          `git diff --name-status origin/${pr_review_base_branch}...HEAD`,
          { encoding: "utf8" },
        ).trim();
      } catch (e) {
        try {
      ...
      let fullGitDiff = "";
      try {
        fullGitDiff = execSync(
          `git diff origin/${pr_review_base_branch}...HEAD -- . ':!*package-lock.json' ':!*yarn.lock' ':!*pnpm-lock.yaml' ':!*Cargo.lock' ':!*go.sum'`,
          { encoding: "utf8" },
        ).trim();
      } catch (e) {
        try {
...
Analyze the changed files, trace references in the codebase, and write your final PR review report in Markdown format.`;
      ...
      cleanup();
      console.error(
        `\x1b[31mError during PR review setup: ${err.message || err}\x1b[0m`,
      );
      playChime("error");
      process.exit(1);
    }
      ...
      for (const file of list) {
        ...
        if (stat.isDirectory()) {
          results = results.concat(
            globFiles(fullPath, maxDepth, currentDepth + 1),
          );
        } else if (stat.isFile()) {
  ...
  function isLikelyCode(text) {
    ...
    if (
      /\b(const|let|var|function|return|import|export|class|await|async)\b/.test(
        trimmed,
      )
    )
      score += 2;
    if (
      /\b(def\s+\w+|elif|import\s+\w+|print\s*\(|if\s+__name__)\b/.test(trimmed)
    )
      score += 2;
    if (
      /\b(public\s+class|private\s+|protected\s+|#include|std::|using\s+namespace)\b/.test(
        trimmed,
      )
    )
      score += 2;
    if (/\b(fn\s+\w+|let\s+mut|pub\s+fn|use\s+std::)\b/.test(trimmed))
      score += 2;
    if (/\b(select\s+.*\s+from|insert\s+into|update\s+.*set)\b/i.test(trimmed))
      score += 2;
    ...
    if (
      /^(hi|hello|please|hey|dear|could you|i want to|can you|explain|what is)\b/i.test(
        trimmed,
      )
    ) {
      score -= 3;
  ...
  function guessLanguage(text) {
    ...
    // Python check
    if (
      /\b(def|class|elif|import)\b/.test(trimmed) &&
    ...
    // TypeScript check
    if (
      /\b(interface|type|as\s+\w+|any|namespace|public|private|readonly)\b/.test(
        trimmed,
      )
    ) {
      return "typescript";
  ...
  if (!is_initial_pr_review) {
    ...
    const vscodeIdx = process.argv.findIndex(
      (arg, i) => i >= 2 && (arg === "-vs" || arg === "--vscode"),
    );
    if (vscodeIdx !== -1) {
      hasVscodeFlag = true;
    ...
    if (hasVscodeFlag) {
      ...
      if (!selectionText || !selectionText.trim()) {
        console.error(
          "\x1b[31mError: No selected text found in VS Code / system clipboard.\x1b[0m",
        );
      ...
      if (detectedLang) {
        try {
          selectionText = await formatCodeWithPrettier(
            selectionText,
            detectedLang,
          );
        } catch (e) {
    ...
    const fileIdx = process.argv.findIndex(
      (arg, i) => i >= 2 && (arg === "-f" || arg === "--file"),
    );
    if (fileIdx !== -1) {
      hasFileFlag = true;
      ...
      if (!fileArg || fileArg.startsWith("-")) {
        console.error(
          "\x1b[31mError: Missing file path for --file (-f) command.\x1b[0m",
        );
        console.error(
          "Usage: nono --file <path>[:line] or --file <path>[:start_line-end_line]",
        );
        process.exit(1);
      }
      ...
      try {
        ...
        console.error(
          `\x1b[31mError reading file ${filePath}: ${err.message}\x1b[0m`,
        );
        process.exit(1);
      }
      ...
      if (detectedLang) {
        try {
          extractedText = await formatCodeWithPrettier(
            extractedText,
            detectedLang,
          );
        } catch (e) {
      ...
      let isLangSupported =
        detectedLang &&
        detectedLang !== "plain" &&
        cliHighlight.supportsLanguage(detectedLang);
      if (isLangSupported) {
        try {
    ...
    const clipboardIdx = process.argv.findIndex(
      (arg, i) => i >= 2 && (arg === "-c" || arg === "--clipboard"),
    );
    if (clipboardIdx !== -1) {
    ...
    if (hasClipboardFlag) {
      ...
      if (guessedLang) {
        try {
          clipboardText = await formatCodeWithPrettier(
            clipboardText,
            guessedLang,
          );
        } catch (e) {
    ...
    if (process.argv[2] === "--write" || process.argv[2] === "-w") {
      const tempPath = path.join(
        os.tmpdir(),
        `nono_prompt_${Date.now()}_temp.txt`,
      );
      try {
        ...
        await new Promise((resolve, reject) => {
          ...
          function trySpawn() {
            if (editorIndex >= editors.length) {
              reject(
                new Error(
                  `None of the editors (${editors.join(", ")}) could be started.`,
                ),
              );
              return;
            }
        ...
        console.error(
          `\x1b[31mError opening/reading temp file in editor: ${err.message}\x1b[0m`,
        );
        process.exit(1);
      }
    ...
    try {
      ...
      while (true) {
        try {
          ...
          if (use_vllm) {
            const systemInstruction = is_initial_pr_review
              ? isCommentMode
                ? pr_review_comment_system_prompt
            ...
            const base_tools = is_initial_pr_review
              ...
              : tools_declarations;
            const vllm_tools = base_tools.concat([
              gemini_web_search_declaration,
            ]);
          ...
          if (response && response.usageMetadata) {
            ...
            latest_tok_speed =
              duration_ms > 0
                ? Math.round(cand_tokens / (duration_ms / 1000))
                : 0;
          ...
          if (
            err_str.includes("--enable-auto-tool-choice") ||
            err_str.includes("tool-call-parser") ||
            err_str.includes("tool choice")
          ) {
            console.error(
              `\n\x1b[31mError: vLLM server is not configured for tool calling!\x1b[0m`,
            );
            console.error(
              `\x1b[33mTo fix this, please restart your vLLM server with these flags:\x1b[0m`,
            );
            console.error(
              `\x1b[36m  --enable-auto-tool-choice --tool-call-parser <hermes|llama|mistral>\x1b[0m\n`,
            );
          }
          const is_transient =
            err_str.includes("503") ||
            err_str.includes("429") ||
            err_str.includes("UNAVAILABLE") ||
            err_str.includes("service is currently unavailable");
          if (is_transient && attempts < maxAttempts) {
            const delay = Math.pow(2, attempts) * 1000;
            updateProgress(
              `• Transient API error encountered (${apiErr.message || apiErr}). Retrying in ${delay / 1000}s (Attempt ${attempts}/${maxAttempts})...`,
            );
      ...
      if (response.usageMetadata) {
        ...
        const cleanPromptText = (currentPrompt || "")
          .split("\n\n[")[0]
          .split("\n[")[0]
          .trim();
      ...
      if (groundingMetadata) {
        if (Array.isArray(groundingMetadata.webSearchQueries)) {
          const queries = groundingMetadata.webSearchQueries.filter(
      ...
      if (pendingSummaryTriggers.length > 0) {
        ...
        if (
          ...
          Array.isArray(last_user_msg.parts)
        ) {
          for (const trigger of pendingSummaryTriggers) {
            const summary = await runSummarizationSubAgent(
              trigger.originalResult,
              query,
            );
            writeDetails(
              `[Summarizer Trigger] Summary generated for ${trigger.name}:\n${summary}`,
            );

            const matching_part = last_user_msg.parts.find(
              (p) =>
                p.functionResponse &&
      ...
      if (!has_function_calls) {
        if (isCommentMode) {
          ...
          const elapsed = Math.round((Date.now() - start_time) / 1000);
          console.log(
            `\x1b[90m• Worked for ${formatElapsedTime(elapsed)}\x1b[0m`,
          );
          ...
          if (
            ...
            if (issueJson.severity) {
              console.log(
                `  \x1b[1mSeverity:\x1b[0m ${issueJson.severity.toUpperCase()}`,
              );
            }
            ...
            if (isAutoMode) {
              lastIssueJson = null;
              const severityStr = issueJson.severity
                ? `**[${issueJson.severity.toUpperCase()}]** `
                : "";
              const commentBody = cleanText
                ? `${severityStr}${cleanText}`
                : `${severityStr}${issueJson.message}`;
              prComments.push({
                path: issueJson.file,
                line: issueJson.line,
                body: commentBody,
              ...
              console.log(
                `\x1b[32mAutomatically saved comment for ${issueJson.file}:${issueJson.line}.\x1b[0m\n`,
              );
            } else {
              let validChoice = false;
              while (!validChoice) {
                const answer = await askUser(
                  "Choose an action [skip (s) / comment (c) / write (w)]: ",
                );
                ...
                if (choice === "s" || choice === "skip") {
                  ...
                  console.log(
                    `\x1b[32mSaved comment for ${issueJson.file}:${issueJson.line}.\x1b[0m\n`,
                  );
                } else if (choice === "w" || choice === "write") {
                  validChoice = true;
                  const promptText = await askUser(
                  ...
                  console.log(
                    '\x1b[31mInvalid choice. Please type "skip", "comment", or "write".\x1b[0m',
                  );
                }
              }
            }
            ...
            console.log(
              "\x1b[33mWarning: Could not parse issue details from response.\x1b[0m",
            );
            if (isAutoMode) {
              lastIssueJson = null;
              history.push({
              ...
              console.log(
                "\x1b[90mAutomatically proceeding to next issue...\x1b[0m\n",
              );
            } else {
              const promptText = await askUser(
                'Enter your prompt / question, or type "next" to continue: ',
              );
          ...
          if (auto_continue && isTaskCompletedMsg && autoContinueCount < 3) {
            ...
            console.log(
              `\x1b[33mAuto-continuing (${autoContinueCount}/3)... \x1b[0m\n`,
            );
  ...
  if (isCommentMode) {
    if (prComments.length > 0) {
      ...
      if (isAutoMode) {
        ...
        const answer = await askUser(
          "Do you want to post the comments and submit requested changes review on the Github PR? (N/y): ",
        );
        if (
      ...
      if (shouldPost) {
        ...
        try {
          ...
          await githubFetch(
            `https://api.github.com/repos/${prOwner}/${prRepo}/pulls/${prPullNumber}/reviews`,
            {
              method: "POST",
          ...
          console.log(
            `\x1b[90mRetrying with individual comment validation fallback...\x1b[0m`,
          );
          ...
          try {
            ...
            const pendingReview = await githubFetch(
              `https://api.github.com/repos/${prOwner}/${prRepo}/pulls/${prPullNumber}/reviews`,
              {
                method: "POST",
                body: JSON.stringify({
            ...
            for (const c of prComments) {
              ...
              try {
                await githubFetch(
                  `https://api.github.com/repos/${prOwner}/${prRepo}/pulls/${prPullNumber}/reviews/${reviewId}/comments`,
                  {
                    method: "POST",
                ...
                console.log(
                  `\x1b[90m  • Attached comment: ${c.path}:${c.line}\x1b[0m`,
                );
                successCount++;
              } catch (commentErr) {
                console.log(
                  `\x1b[31m  • Warning: Skipped comment on ${c.path}:${c.line} (Line not in PR diff or invalid)\x1b[0m`,
                );
              }
            }
            ...
            await githubFetch(
              `https://api.github.com/repos/${prOwner}/${prRepo}/pulls/${prPullNumber}/reviews/${reviewId}/events`,
              {
                method: "POST",
                body: JSON.stringify({
                  event: "REQUEST_CHANGES",
            ...
            console.error(
              `\x1b[31mError submitting review during fallback: ${fallbackErr.message || fallbackErr}\x1b[0m`,
            );
            playChime("error");
          }
        }
