...
function getPgrpAndTpgid(pid) {
  try {
    ...
    const fields = stat.substring(last_paren + 2).trim().split(/\s+/);
    return {
...
---`
  ...
  if (has_executed_command) {
    // If a command was executed, a new shell prompt has been printed automatically by the shell.
    // We just query its new column.
    setTimeout(() => {
      queryCursorPosition();
    }, 100);
  } else {
    // If no command was executed, reprint the saved shell prompt to return to original state
    process.stdout.write(last_pty_line);
  }
}
  ...
  return new Promise((resolve) => {
    ...
    command_check_interval = setInterval(() => {
      if (isShellInForeground()) {
        ...
        if (idle_time >= 75) { // Ensure shell has finished printing prompt
